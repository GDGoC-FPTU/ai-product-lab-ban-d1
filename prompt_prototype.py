...
def evaluate_prompt(user_input: str) -> str:
    ...
    if not api_key:
        # No API key available — return simulated boundary-enforcement responses
        import re as _re
        battery_match = _re.search(r'(\d+)\s*%', user_input)
        ...
        return "[DRAFT_ONLY] Kính gửi quý khách, chúc quý khách đi đường bình an và an toàn!"
...
if __name__ == "__main__":
    api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
    if not api_key:
        print("[Warning] GEMINI_API_KEY not set. Running in simulation mode.")
